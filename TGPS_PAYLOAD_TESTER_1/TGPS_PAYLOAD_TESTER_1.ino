...
#define TGPS_TEST          0x78  // KNACKSAT_II - TGPS ,TEST REGISTER ADDRESS WITH PAYLOAD
...
bool          BUS_SELECTED    = 0            ;  // BUS_SELECTED TELL WHICH BUS PART ARE CURRENTLY SELECTING.
...
void loop1()
{
  if (MISSION_RUN == true)
  {
    ...
    Serial.print  (">>> BUS PART 1 : "); PRINT(BUS_PART_1);
    Serial.print  (">>> BUS PART 2 : "); PRINT(BUS_PART_2);
    Serial.println("==========================================================");
...
void TGPS_REQUEST()
{
  ...
  {
    SEPARATE_BUS_DATA();
    BUS_SELECTED = EEPROM.read(256);
    if (BUS_SELECTED == 0) Wire.write(BUS_PART_1, 128);
    if (BUS_SELECTED == 1) Wire.write(BUS_PART_2, 128);
    BUS_SELECTED = !BUS_SELECTED;
    EEPROM.write(256, BUS_SELECTED);   // WRITE BUS DATA TO FLASH MEMORY
...
// SEPARATE_BUS_DATA : SEPARATE BUS DATA FOR USING WITH COMMUNICATION.

void SEPARATE_BUS_DATA()
{
  for (int i = 0; i < 125; i++) {
    if ((unsigned long)(millis() - PREVIOUS_TIME_1) >= SET_TIME_OUT) break;
    BUS_PART_1[i] = BUS[i] ;
    BUS_PART_2[i] = BUS[i + 125] ;
  }
  BUS_PART_1[127] = 0x00 ;
  BUS_PART_2[127] = 0x01 ;
}
