...
void CHECKTEXT(bool CHIP_SELECT)
{
  ...
  {
    Serial.println("TIME OUT SET AT " + String(SET_TIME_OUT_SD) + " MILLISECOND");
    ...
    if ((LASTED_FILE_CS1 > 1000) || (LASTED_FILE_CS2 > 1000))   // SETUP FLASH MEMORY FOR MISSION
    {
      Serial.println(">>> SETUP FLASH MEMORY FOR MISSION");
      EEPROM.put(0, 0x00000000);
  ...
  while (true)
  {
    ...
    if (LASTED_FILE_CS1 <= 1000)LASTED_FILE_CS1++   ;   // NEXT FILE OF SD CARD NO.1
    if (LASTED_FILE_CS2 <= 1000)LASTED_FILE_CS2++   ;   // NEXT FILE OF SD CARD NO.2
