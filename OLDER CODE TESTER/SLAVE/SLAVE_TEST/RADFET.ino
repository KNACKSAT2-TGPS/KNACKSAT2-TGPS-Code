...
void READ_RADFET(uint8_t S, uint16_t RADFET_ARRAY[16])
{
  ...
  while (i < 16) {
    if (STATE_REQUEST != pow(2,S))break;
    if (millis() - READ_TIME >= 1000) {
      digitalWrite(S2 , S >> 2 & 1); digitalWrite(S1 , S >> 1 & 1); digitalWrite(S0 , S & 1);  RADFET_ARRAY[i] = analogRead(A);  Serial.print(String(RADFET_ARRAY[i]) + " ");
      i++;
      READ_TIME = millis();
    }
  }
